 ...
 * It is recommended to use the helper function
 * [getCMakeToolsApi](#getCMakeToolsApi) instead of querying the extension
 * instance directly.
 */
...
export type TargetType = 'STATIC_LIBRARY'|'MODULE_LIBRARY'|'SHARED_LIBRARY'|
    'OBJECT_LIBRARY'|'EXECUTABLE'|'UTILITY'|'INTERFACE_LIBRARY';
...
export interface Target {
   ...
   * The value is one of EXECUTABLE, STATIC_LIBRARY, SHARED_LIBRARY,
  ...
  /**
   * The file groups describe a list of compilation information for artifacts of
   * this target. The file groups contains source code files that use the same
   * compilation information and are known by CMake.
   */
 ...
 */
export async function getCMakeToolsApi(desiredVersion: Version,
                                       exactMatch = false):
    Promise<CMakeToolsApi|undefined> {
  const extension =
      vscode.extensions.getExtension('KylinIdeTeam.kylin-cmake-tools');

  if (!extension) {
    console.warn(
        '[vscode-cmake-tools-api] CMake Tools extension is not installed, Please install extension `KylinIdeTeam.kylin-cmake-tools` first.');
    return undefined;
  }
  ...
  if (desiredVersion !== api.version) {
    if (exactMatch) {
      console.warn(`[vscode-cmake-tools-api] CMake Tools API version ${
          desiredVersion} is not available.`);
      return undefined;
    } else {
      console.warn(`[vscode-cmake-tools-api] CMake Tools API version ${
          desiredVersion} is not available. Using ${api.version}.`);
    }
  }
